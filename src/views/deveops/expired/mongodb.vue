<template>
  <div class="manager-mongodb-container">
    ...
    <el-table :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column width="260px" align="center" label="Aliyun">
        <template slot-scope="mongodb">
          <span>{{ mongodb.row.recognition_id }}</span>
        </template>
      </el-table-column>

      <el-table-column width="180px" align="center" label="状态">
        <template slot-scope="mongodb">
          <el-tag :type="mongodb.row.status | statusFilter">{{ mongodb.row.status }}</el-tag>
        </template>
      </el-table-column>

     <el-table-column width="370px" align="center" label="实例名称">
        <template slot-scope="mongodb">
          <span>{{ mongodb.row.instancename }}</span>
        </template>
      </el-table-column>

      <el-table-column width="220px" align="center" label="MongoDB版本">
        <template slot-scope="mongodb">
          <span>{{ mongodb.row.version }}</span>
        </template>
      </el-table-column>

      <el-table-column width="250px" align="center" label="过期时间">
        <template slot-scope="mongodb">
          <span style="color:#FF0000">{{ mongodb.row.expired }}天</span>
        </template>
      </el-table-column>
...
<script>
  import { fetch_ExpiredAliyunMongoDBList } from '@/api/dashboard';
  export default {
      data(){
        return{
          list: null,
      ...
      methods:{
        init(){
          fetch_ExpiredAliyunMongoDBList(this.pagination).then((response)=>{
