<template>
  <div class="manager-kvstore-container">
    ...
    <el-table :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column width="260px" align="center" label="Aliyun">
        <template slot-scope="kvstore">
          <span>{{ kvstore.row.recognition_id }}</span>
        </template>
      </el-table-column>

      <el-table-column width="180px" align="center" label="状态">
        <template slot-scope="kvstore">
          <el-tag :type="kvstore.row.status | statusFilter">{{ kvstore.row.status }}</el-tag>
        </template>
      </el-table-column>

     <el-table-column width="370px" align="center" label="实例名称">
        <template slot-scope="kvstore">
          <span>{{ kvstore.row.instancename }}</span>
        </template>
      </el-table-column>

      <el-table-column width="220px" align="center" label="KVStore版本">
        <template slot-scope="kvstore">
          <span>{{ kvstore.row.version }}</span>
        </template>
      </el-table-column>

      <el-table-column width="250px" align="center" label="过期时间">
        <template slot-scope="kvstore">
          <span style="color:#FF0000">{{ kvstore.row.expired }}天</span>
        </template>
      </el-table-column>
...
<script>
  import { fetch_ExpiredAliyunKVStoreList } from '@/api/dashboard';
  export default {
      data(){
        return{
          list: null,
      ...
      methods:{
        init(){
          fetch_ExpiredAliyunKVStoreList(this.pagination).then((response)=>{
