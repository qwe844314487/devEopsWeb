  ...
  <div class="manager-host-container">
    <div class="filter-container">
      <el-row style="margin-bottom:20px;">
        ...
        <el-button class="filter-item" @click="handleSelectHost()" style="margin-left: 10px;" type="primary" icon="el-icon-goods" :disabled="btnStatus">归类</el-button>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogSelectGroupVisible" width="60%" top="2vh">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogSelectGroupVisible = false" :disabled="btnStatus">取消</el-button>
        <el-button type="primary" @click="handleQRCode" :disabled="btnStatus">提交</el-button>
      </div>
    </el-dialog>
...
export default {
  data() {
    return {
      ...
      rules: {
        info: [
          { required: true, message: "主机信息是必须的", trigger: "blur" }
        ],
        connect_ip: [
          {
            required: true,
  ...
  methods: {
    ...
    handleSelectGroup(row) {
      this.commit_obj = Object.assign({}, row) // copy obj
      this.dialogStatus = "selectgroup"
      this.dialogSelectGroupVisible = true
      this.$nextTick(() => {
        ...
        .then((response) => {
          if (response.data.isexpire) {
            ...
            if (this.dialogStatus === "create") {
              ...
              this.updateData()
            } else if (this.dialogStatus === "selecthost"){
              this.selectHost()
            } else if (this.dialogStatus === 'selectgroup'){
              this.selectGroup()
